...
-- Repo-object read grant. Run this AFTER the git repositories exist
-- (account/13-14). The per-repo form is always valid; the ALL/FUTURE bulk
-- form is NOT confirmed for GIT REPOSITORIES - use per-repo if it is rejected.
-- GRANT USAGE ON GIT REPOSITORY PLATFORM_DB.DEPLOYMENT.BB_TBAYTEL_REPO TO ROLE IDENTIFIER($ENV_DEPLOYER);
-- GRANT USAGE ON GIT REPOSITORY PLATFORM_DB.DEPLOYMENT.AZDO_REPO       TO ROLE IDENTIFIER($ENV_DEPLOYER);
