...
-- No object depends on this at creation time; the stage references it only at
-- COPY time (passed via the load proc's P_OTHER), so just create it before first load.

use role sysadmin;                 -- owner of PLATFORM_DB
use database platform_db;

create or replace file format platform_db.file_formats.ff_parquet
    type = parquet
