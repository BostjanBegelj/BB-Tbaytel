...
-- Computes DURATION_MSEC from start/end. Kept minimal and handler-free so callers
-- can wrap the CALL in a guarded block (logging must never mask a real failure).
...
RETURNS NUMBER(38,0)
LANGUAGE SQL
COMMENT = 'Helper: insert one ADM.PPN_LOG row. Returns rows inserted (1).'
EXECUTE AS CALLER
AS
DECLARE
...
BEGIN
    ...
    VALUES (
        :P_PPN_ID, :P_RUN_ID, :P_SOURCE_ID, :P_TABLE_NAME, :P_PHASE, :P_STATUS,
